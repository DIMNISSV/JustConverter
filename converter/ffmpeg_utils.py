...
import time
import math
from tkinter import messagebox # Keep for utils if needed, GUI handles main interaction
from .exceptions import FfprobeError, CommandGenerationError, PreprocessingError, ConversionError, FfmpegError
...
def run_ffprobe(command):
    ...
    try:
        ...
        raise FfprobeError("ffprobe не найден. Убедитесь, что FFmpeg (включая ffprobe) установлен и добавлен в системный PATH.")
...
def get_essential_stream_params(file_path):
    ...
    try:
        ...
        cmd_video = [
            ...
            "-show_entries", "stream=width,height,pix_fmt,sample_aspect_ratio,display_aspect_ratio,r_frame_rate,time_base",
            "-of", "json", file_path
        ]
        ...
        if not params['sample_rate'] and params.get('time_base_a') is None: # Check if audio stream was likely present but failed
...
def generate_ffmpeg_commands_concat(
    input_file, output_file, encoding_params_str, main_video_params, main_video_duration,
    embed_ads, banner_file, banner_timecodes, moving_file, track_data):
    """
    ...
    def create_segment_command(input_path, output_path, start_time=None, duration=None, is_ad=False):
        ...
        if target_v_timebase and '/' in target_v_timebase:
             ...
             except ValueError: pass # Keep default if eval fails
        ...
        cmd_parts = [
            ...
            *(["-af", f"aresample={target_sample_rate},"
                      f"aformat=sample_fmts={target_sample_fmt}:channel_layouts={target_channel_layout}"] if has_audio else ["-an"]),
    ...
    if banner_file and banner_timecodes and banner_input_stream:
        ...
        if sorted_embed_ads:
             current_time_in_output = 0.0; original_time_processed = 0.0; ad_idx = 0
             for banner_original_sec in sorted_banner_timecodes_sec:
                 while ad_idx < len(sorted_embed_ads):
                      ad = sorted_embed_ads[ad_idx]; ad_original_sec = timecode_to_seconds(ad['timecode'])
                      if ad_original_sec < banner_original_sec:
                          current_time_in_output += (ad_original_sec - original_time_processed) + ad.get('duration', 0)
                          original_time_processed = ad_original_sec; ad_idx += 1
                      else: break
                 current_time_in_output += (banner_original_sec - original_time_processed)
                 original_time_processed = banner_original_sec
                 start_time = current_time_in_output; end_time = min(start_time + banner_duration, final_duration_estimate)
                 if start_time < final_duration_estimate - 0.01: enable_parts.append(f"between(t,{start_time:.3f},{end_time:.3f})")
        else:
             for banner_original_sec in sorted_banner_timecodes_sec:
                 start_time = banner_original_sec; end_time = min(start_time + banner_duration, final_duration_estimate)
                 if start_time < final_duration_estimate - 0.01: enable_parts.append(f"between(t,{start_time:.3f},{end_time:.3f})")

        # Add overlay filter part if enable times were generated
        if enable_parts:
            enable_expression = "+".join(enable_parts)
            overlay_y_pos = f"{target_h if target_h else 'main_h'}-overlay_h-10" # Use target_h or main_h
            filter_complex_parts.append(
                 f"{current_video_input_for_filter}{scaled_banner_stream}"
                 f"overlay=x=10:y={overlay_y_pos}:enable='{enable_expression}'"
    ...
    # Moving Ad Overlay Filter
    if moving_file and moving_input_stream:
        scaled_moving_stream = f"[moving_scaled_{moving_input_index}]" # Unique label
        overlay_output_label_moving = f"[v_moving_out_{moving_input_index}]" # Unique label

        # Add scaling/SAR filter part
        filter_complex_parts.append(
            f"{moving_input_stream}scale=-1:h={(target_h or 720)}/10:flags=bicubic," # Use target_h or default
            f"setsar={target_sar if target_sar else '1/1'}{scaled_moving_stream}"
        )

        # Calculate animation expressions
        T = final_duration_estimate; W = target_w or 1280; H = target_h or 720
        if T <= 0:
             print("Предупреждение: Оценочная длительность видео равна нулю, анимация движ. рекламы отключена.")
        else:
            t_seg1 = T / 4.0; t_seg2 = T / 2.0; t_seg3 = 3.0 * T / 4.0
            # Clamp segment lengths to prevent division by zero if T is very small
            len1 = max(t_seg1, 1e-6); len2 = max(t_seg2 - t_seg1, 1e-6)
            len3 = max(t_seg3 - t_seg2, 1e-6); len4 = max(T - t_seg3, 1e-6)
            x_expr = (f"if(lt(t,{t_seg1:.6f}), t/{len1}*(W-overlay_w),"
                      f" if(lt(t,{t_seg2:.6f}), W-overlay_w,"
                      f"  if(lt(t,{t_seg3:.6f}), (W-overlay_w)*(1-(t-{t_seg2:.6f})/{len3}),"
                      f"   0)))")
            y_expr = (f"if(lt(t,{t_seg1:.6f}), 0,"
                      f" if(lt(t,{t_seg2:.6f}), (H-overlay_h)*(t-{t_seg1:.6f})/{len2},"
                      f"  if(lt(t,{t_seg3:.6f}), H-overlay_h,"
                      f"   (H-overlay_h)*(1-(t-{t_seg3:.6f})/{len4}))))")

            # Add overlay filter part
            filter_complex_parts.append(
                 f"{current_video_input_for_filter}{scaled_moving_stream}"
                 f"overlay=x='{x_expr}':y='{y_expr}'{overlay_output_label_moving}"
            )
