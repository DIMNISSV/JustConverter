...
_BANNER_GAP_COLOR = "black@0.0" # Transparent black for gaps
...
def get_media_duration(file_path: str) -> Optional[float]:
    ...
    try:
        ...
        if duration_str_fmt and duration_str_fmt != "N/A":
            try: duration = float(duration_str_fmt)
        ...
        if duration is None or duration <= 0:
            ...
            except FfprobeError: pass

        if duration and duration > 0.01: return duration
        else: return None

    except FfprobeError: return None
    except Exception as e: print(f"Неожиданная ошибка в get_media_duration для {file_path}: {e}"); return None


def get_stream_info(file_path: str) -> Dict[str, Any]:
    ...
    except FfprobeError: return {}


def get_essential_stream_params(file_path: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        cmd_video = [ "ffprobe", "-v", "error", "-select_streams", "v:0",
            "-show_entries", "stream=width,height,pix_fmt,sample_aspect_ratio,display_aspect_ratio,r_frame_rate,time_base,codec_name",
            "-of", "json", file_path ]
        data_v = run_ffprobe(cmd_video)
        if data_v.get("streams"):
            stream_v = data_v["streams"][0]
            params['width'] = stream_v.get('width'); params['height'] = stream_v.get('height')
            ...
            params['time_base_v'] = stream_v.get('time_base'); params['fps_str'] = stream_v.get('r_frame_rate')
            if all([params['width'], params['height'], params['fps_str'], params['time_base_v']]):
                 has_video_stream = True
                 if not params['pix_fmt']: params['pix_fmt'] = 'yuv420p'
    except FfprobeError: pass
    except Exception as e: print(f"Неожиданная ошибка при зондировании видеопотока {file_path}: {e}")
        ...
        try:
             ...
             if any(fmt in format_name for fmt in image_formats):
                 ...
                 if data_img_s.get("streams"):
                     ...
                     params['width'] = stream_img.get('width'); params['height'] = stream_img.get('height')
    ...
    try:
        ...
        if data_a.get("streams"):
            ...
            params['sample_rate'] = stream_a.get('sample_rate'); params['channel_layout'] = stream_a.get('channel_layout')
            params['sample_fmt'] = stream_a.get('sample_fmt'); params['time_base_a'] = stream_a.get('time_base')
            if all([params['sample_rate'], params['channel_layout'], params['sample_fmt'], params['time_base_a']]):
                params['has_audio'] = True
            else: params['has_audio'] = False
    except FfprobeError: pass
...
def _calculate_adjusted_times(original_time_sec: float, is_concat_mode: bool, sorted_embed_ads: List[Dict]) -> float:
    ...
    adjusted_time = 0.0; last_original_time = 0.0
...
def _validate_and_prepare_inputs(input_file: str, output_file: str, main_video_params: Optional[Dict],
                                 ...
                                 ) -> Tuple[Dict, float, List[Dict], Optional[str], Optional[List[str]], Optional[str], Optional[float]]:
    ...
    if banner_file and banner_timecodes:
        if os.path.exists(banner_file):
            original_banner_duration = get_media_duration(banner_file)
            if original_banner_duration is None:
                original_banner_duration = 5.0
                print(f"Предупреждение: Не удалось определить длительность баннера '{os.path.basename(banner_file)}', используется {original_banner_duration:.1f}s (возможно, изображение).")
        ...
        else: print(f"Предупреждение: Файл баннера не найден '{banner_file}', будет проигнорирован.")
...
def _create_segment_command(input_path: str, output_path: str, target_params: Dict,
    ...
    if is_banner_segment:
        banner_params = get_essential_stream_params(input_path)
        ...
        final_pix_fmt = _BANNER_TRACK_PIX_FMT
    else:
        vf_parts.extend([
    ...
    cmd_parts = ["ffmpeg", "-y"]
    input_options = []
    is_image_input = get_media_duration(input_path) is None
...
def _generate_preprocessing_for_concat(input_file: str, sorted_embed_ads: List[Dict], target_params: Dict,
    ...
    except IOError as e: raise CommandGenerationError(f"Не удалось создать файл списка concat для видео+рекламы: {e}")

    return preprocessing_commands, concat_list_path, temp_files_generated, total_ad_duration_sum


# --- Generate Banner Preprocessing ---
def _generate_banner_preprocessing_commands(
    banner_file: str, banner_timecodes: List[str], original_banner_duration: float,
    ...
    banner_params = get_essential_stream_params(banner_file)
    banner_scaled_height = target_params['height'] // 10
    if banner_params and banner_params.get('width') and banner_params.get('height'):
        orig_w, orig_h = banner_params['width'], banner_params['height']
        banner_scaled_height = max(1, int(orig_h * (banner_scaled_width / orig_w))) if orig_w > 0 else banner_scaled_height
    print(f"  Определены размеры трека баннера: {banner_scaled_width}x{banner_scaled_height}")

    temp_banner_segment_path = _generate_temp_filename("banner_segment_uniq", segment_counter)
    banner_segment_cmd = _create_segment_command(
    ...
    for banner_original_sec in valid_banner_timecodes_sec:
        adjusted_start = _calculate_adjusted_times(banner_original_sec, is_concat_mode, sorted_embed_ads)
    ...
    for i, (start_time, end_time) in enumerate(adjusted_banner_times_sec):
        ...
        if gap_duration > 0.001:
            temp_gap_path = _generate_temp_filename("banner_gap", segment_counter)
            gap_cmd_parts = [
                "ffmpeg", "-y", "-f", "lavfi",
                "-i", f"color=c={_BANNER_GAP_COLOR}:s={banner_scaled_width}x{banner_scaled_height}:d={gap_duration:.6f}:r={target_params['fps_str']}",
                "-vf", f"format=pix_fmts={_BANNER_TRACK_PIX_FMT}",
                "-c:v", _TEMP_VIDEO_CODEC, "-preset", _TEMP_VIDEO_PRESET, "-crf", "0",
        ...
        with open(banner_concat_list_path, 'w', encoding='utf-8') as f:
            ...
            for item in concat_list_items:
                if '\n' in item:
                     ...
                     f.write(f"file {_escape_path_for_concat(path_part)}\n")
                     f.write(f"{duration_part}\n")
                else:
                    f.write(f"file {_escape_path_for_concat(item)}\n")
        print(f"  Создан файл списка: {banner_concat_list_path}")
    except IOError as e: raise CommandGenerationError(f"Не удалось создать файл списка concat для баннера: {e}")

    print(f"--- Фаза 2.3: Генерация команды для конкатенации трека баннера ---")
    concatenated_banner_path = _generate_temp_filename("banner_track_final", 0)
...
def _build_filter_complex(
    ...
    if concatenated_banner_track_idx is not None and banner_timecodes and original_banner_duration is not None:
        try:
            print(f"  Настройка фильтра overlay для трека баннера (Input: [{concatenated_banner_track_idx}:v], с between)...")
            ...
            valid_banner_timecodes_sec = sorted(filter(None, [_timecode_to_seconds(tc) for tc in banner_timecodes]))
            for banner_original_sec in valid_banner_timecodes_sec:
                adjusted_start_time = _calculate_adjusted_times(banner_original_sec, is_concat_mode, sorted_embed_ads)
            ...
            if enable_parts:
                ...
                print(f"    Фильтр overlay для трека баннера (с between) добавлен. Выход: {last_filter_video_label}")
    ...
    print(f"--- Итоговый filter_complex сгенерирован ({len(all_filter_parts)} этапов). Видео выход: [{final_video_output_map_label}] ---")
...
def _generate_main_ffmpeg_command(
    ...
    if is_concat_mode:
        if not concat_list_path or not os.path.exists(concat_list_path): raise CommandGenerationError("Concat list (main) not found.")
    ...
    current_input_index = len(input_definitions); banner_track_input_idx = None; moving_input_idx = None
    if concatenated_banner_track_path:
        banner_options = []
        print(f"  Input {current_input_index} (Concatenated Banner Track): {os.path.basename(concatenated_banner_track_path)}")
        input_definitions.append((banner_options, concatenated_banner_track_path)); banner_track_input_idx = current_input_index; current_input_index += 1
    if moving_file and os.path.exists(moving_file):
        moving_options = ["-loop", "1"]
        if get_media_duration(moving_file) is None: moving_options.extend(["-r", target_params['fps_str']])
    ...
    if source_file_for_metadata and os.path.exists(source_file_for_metadata):
        ...
        for map_cmd in map_commands:
             ...
             if is_filter_map:
                ...
                if in_idx == metadata_input_index:
                    ...
                    if stream_type == 'v': stream_map[original_spec_key] = (f"s:v:{out_v_idx}"); out_v_idx += 1
                    elif stream_type == 'a': stream_map[original_spec_key] = (f"s:a:{out_a_idx}"); out_a_idx += 1
                    elif stream_type == 's': stream_map[original_spec_key] = (f"s:s:{out_s_idx}"); out_s_idx += 1
        ...
        for track_id_from_user, edits in track_data.items():
            ...
            norm_track_id = None
            if len(parts) == 2: norm_track_id = f"{metadata_input_index}:{parts[0]}:{parts[1]}"
            elif len(parts) == 3: norm_track_id = track_id_from_user
            else: continue
    ...
    if output_file.lower().endswith(".mp4"):
        movflags_val = "+faststart"; movflags_present = False; temp_cmd_parts = []
        skip_next = False
...
def generate_ffmpeg_commands(
    ...
    try:
        ...
        original_banner_duration = _validate_and_prepare_inputs(
    ...
    if banner_file and banner_timecodes and original_banner_duration is not None:
        try:
            prep_cmds_banner, _, prep_temp_files_banner, concatenated_banner_path = \
                _generate_banner_preprocessing_commands(
        ...
        except CommandGenerationError as e: _cleanup_temp_files(all_temp_files); print(f"Ошибка генерации трека баннера: {e}"); raise e
        except Exception as e: _cleanup_temp_files(all_temp_files); print(f"Неожиданная ошибка генерации трека баннера: {e}"); raise CommandGenerationError(f"Ошибка трека баннера: {e}") from e
    else:
    ...
    except Exception as e: _cleanup_temp_files(all_temp_files); print(f"Неожиданная ошибка генерации основной команды: {e}"); raise CommandGenerationError(f"Ошибка основной команды: {e}") from e
...
def run_ffmpeg_command(cmd: str, step_name: str):
    ...
    except ConversionError as e: raise e
...
def _cleanup_temp_files(temp_files: List[str]):
    ...
    for f in list(temp_files):
        ...
        except OSError as e: print(f"  Ошибка удаления {os.path.basename(f)}: {e}"); failed_count += 1
        except Exception as e: print(f"  Неожиданная ошибка при удалении {os.path.basename(f)}: {e}"); failed_count += 1
